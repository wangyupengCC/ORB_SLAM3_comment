...
int main(int argc, char *argv[])
{
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        cout << "Loading IMU for sequence " << seq << "..."<<std::endl;
...
void LoadIMU(const string &strImuPath, vector<double> &vTimeStamps, vector<cv::Point3f> &vAcc, vector<cv::Point3f> &vGyro)
{
    ...
    std::cout<<"strImuPath = "<<strImuPath<<std::endl;
    ...
    while(!fImu.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            while ((pos = s.find(',')) != string::npos) {
                // std::cout<<"the character string has ,"<<std::endl;
                item = s.substr(0, pos);
    ...
    std::cout<<"/********************Loading IMU done********************/"<<std::endl;
}
