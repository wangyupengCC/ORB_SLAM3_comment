...
namespace ORB_SLAM3 {
    ...
    cv::Point2f Mei::project(const cv::Point3f &p3D) {
        const float x2_plus_y2_plus_z2 = p3D.x * p3D.x + p3D.y * p3D.y + p3D.z * p3D.z;
        const float Nc = (sqrtf(x2_plus_y2_plus_z2));
        //归一化球面坐标
        const cv::Point3f Ps_norm = (p3D.x/Nc,p3D.y/Nc,p3D.z/Nc);
        //球面坐标
        const cv::Point3f Ps = (p3D.x/Nc,p3D.y/Nc,(p3D.z + mvParameters[0])/Nc);
        //归一化平面坐标
        const cv::Point3f uv_norm = (p3D.x/(p3D.z+mvParameters[0]*N),p3D.y/(p3D.z+mvParameters[0]*N),1.0);
        //像素坐标
        const cv::Point2f uv = 
